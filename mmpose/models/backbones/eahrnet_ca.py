...
class eca_layer(nn.Module):
    ...
    Paper 2020CVPR ECA-Net: Efficient Channel Attention for Deep Convolutional Neural Networks  arxiv: https://arxiv.org/abs/1910.03151
    Args:
    ...
    def forward(self, x):
        ...
        y = self.avg_pool(x) #y.shape=[b,c,1,1]
        # Two different branches of ECA module  #[b,c,1,1]->[b,c,1]->[b,1,c]进行卷积后回到原尺寸->[b,c,1]->[b,c,1,1]
        y = self.conv(y.squeeze(-1).transpose(-1, -2)).transpose(-1, -2).unsqueeze(-1)
        # Multi-scale information fusion
        y = self.sigmoid(y)

class ECASpatialWeighting(nn.Module): #using ECANet
    ...
    def forward(self, x):
        # b, c, h, w = x.size() # x: input features with shape [b, c, h, w]
        out = self.global_avgpool(x) # feature descriptor on the global spatial information
        out = self.conv(out.squeeze(-1).transpose(-1, -2)).transpose(-1, -2).unsqueeze(-1) # Two different branches of ECA module
...
class SqueezeExcite(nn.Module):
    def __init__(self, in_chs, se_ratio=0.25, reduced_base_chs=None,
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.conv_reduce = nn.Conv2d(in_chs, reduced_chs, 1, bias=False) #原实现中bias=True
        self.act1 = act_layer(inplace=True)
        self.conv_expand = nn.Conv2d(reduced_chs, in_chs, 1, bias=False) #原实现中bias=True
...
class Stem_ghostBottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        self.ghostbottleneck=GhostBottleneck(stem_channels, self.out_channels, self.out_channels, 3, 2, se_ratio=0.25) #使用了SENet

    def forward(self, x):

        def _inner_forward(x):
            x = self.conv1(x)
            out = self.ghostbottleneck(x)
            return out
